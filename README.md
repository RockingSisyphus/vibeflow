...
VibeFlow 强制 AI 先构建程序流程图，再逐步细化 nodeset、node、base_lib、plugin 和 config。它把“先想清楚结构，再写代码”变成项目级硬约束，而不是靠提示词祈祷 AI 自觉。

它还强制 AI 遵守高内聚、低耦合、小文件、小函数、显式流程边和可检查契约。AI 仍然可以高速开发，但每一轮修改都必须回到可视化、可校验、可运行的流程图里。

它不是一个新的聊天工具，也不是一个更复杂的框架。它的目标很直接：让 vibe coding 不再越写越散、越改越乱。
