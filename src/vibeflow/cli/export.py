...
def handle_export_graph(args, *, export_kind: str) -> int:
    ...
    if getattr(args, "workspace", None):
        loaded = _workspace_graph_or_report(args)
        ...
        registry = None
        try:
            graph, compiled, resources = _load_legacy_graph(args)
...
def _workspace_graph_or_report(args):
    from vibeflow.health.types import HealthReport
    ...
    if isinstance(workspace, HealthReport):
        return workspace
    graph, compiled, registry, resources, error = load_workspace_graph_for_export(Path(args.config), workspace=workspace)
