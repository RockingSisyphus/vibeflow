"""Language-neutral nodeset dependency and nesting analysis."""
...
def analyze_nodeset_dependencies(graph: GraphConfig) -> NodesetDependencyGraph:
    return NodesetDependencyGraph(
        pipeline=_dependencies_for_nodes(
            graph.nodes,
            graph=graph,
            owner="pipeline",
        ),
        nodesets={
            name: _dependencies_for_nodes(
                nodeset.graph.nodes,
                graph=graph,
                owner=f"nodeset:{name}",
            )
...
def nodeset_dependency_cycles(
    ...
    for root in sorted(adjacency):
        if state.get(root, 0) != 0:
            continue
        cycles.update(
            _cycles_from_root(root, adjacency=adjacency, state=state)
        )

    return tuple(sorted(cycles))


def nodeset_depth_violations(
    graph: GraphConfig,
    *,
    max_depth: int,
) -> tuple[NodesetDepthViolation, ...]:
    ...
    *,
    ...
    for node in nodes:
        if node.type_used in LOOP_NODE_TYPES and node.loop.body:
            dependencies.append(
                NodesetDependency(owner, node.id, node.loop.body, "loop_body")
            )
        elif node.type_used in graph.nodesets:
            dependencies.append(
                NodesetDependency(owner, node.id, node.type_used, "nodeset")
            )
            ...
            sorted(
                {
                    ...
                    if dependency.target in dependencies.nodesets
                }
            )
        )
    ...
    *,
    ...
    while stack:
        ...
        if name in memo or name in cyclic_nodes:
            continue
        children = _acyclic_children(
            name,
            dependencies=dependencies,
            cyclic_nodes=cyclic_nodes,
        )
        if not expanded:
            _queue_longest_path_children(
                name,
                children=children,
                memo=memo,
                stack=stack,
...
def _acyclic_children(
    ...
    *,
    ...
    return tuple(
        ...
        if dependency.target in dependencies.nodesets
        and dependency.target not in cyclic_nodes
    )
...
def _reachable_from_pipeline(
    ...
    while stack:
        ...
        stack.extend(
            dependency.target
            for dependency in reversed(dependencies.nodesets.get(name, ()))
        )
    return reachable


def _dependency_key(
    dependency: NodesetDependency,
...
def _path_key(path: NodesetPath) -> tuple[object, ...]:
    chain, calls = path
    reversed_calls = tuple(reversed(tuple(_dependency_key(call) for call in calls)))
    return len(chain), tuple(reversed(chain)), reversed_calls
