"""Language-neutral configuration scope composition."""
...
def normalize_config_scope(
    raw: Mapping[str, Any] | ConfigScope | None,
) -> ConfigScope:
    if raw is None:
        return ConfigScope()
    ...
    allow = _bool_field(
        raw.get(
            ALLOW_CONFIG_OVERRIDE_FIELD,
            raw.get(OVERRIDE_CHILD_CONFIG_FIELD, False),
        )
    )
...
def node_invocation_scope(
    ...
    *,
    ...
    return ConfigScope(
        ...
        allow_config_override=bool(allow_config_override),
    )
...
def merge_config_scopes(base: ConfigScope, override: ConfigScope) -> ConfigScope:
    if not override.values:
        return base
    return ConfigScope(
        values={**dict(base.values), **dict(override.values)},
        allow_config_override=override.allow_config_override,
    )
    ...
    for key, item in value.items():
        if not isinstance(item, Mapping):
            raise ConfigBindingError(
                f"node config override for '{key}' must be an object"
            )
        ...
        selected.update(
            {
                ...
                if str(key) not in SCOPED_CONFIG_RESERVED_FIELDS
            }
        )
    else:
        selected.update(
