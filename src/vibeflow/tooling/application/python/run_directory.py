"""Run-directory ownership at the application/filesystem boundary."""
...
def validate_run_id(run_id: object) -> str:
    ...
    if (
        ...
        raise InvalidRunIdError(
            "run id must be one non-empty path component without "
            "'.'/'..' or path separators"
        )
    return value
...
def prepare_run_dir(run_root: str | Path | None, run_id: object) -> Path:
    ...
    run_dir = (
        Path(run_root) if run_root is not None else Path("runs")
    ) / safe_run_id
    try:
