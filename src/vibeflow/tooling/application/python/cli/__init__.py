...
def build_parser() -> argparse.ArgumentParser:
    ...
    from vibeflow.tooling.application.python.cli.delegate_cli import add_delegate_cli_parser

    add_delegate_cli_parser(sub, _add_runtime_options)

    from vibeflow.tooling.application.python.cli.review import add_review_parser

    add_review_parser(sub)

    from vibeflow.tooling.application.python.cli.quality import add_quality_parser
...
def _handle_validate(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "workspace", None):
        ...
        from vibeflow.tooling.application.python.workspace_service import validate_workspace_config_path
...
def _handle_inspect_node(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.python.cli.node import inspect_node_payload
...
def _handle_inspect_config(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.python.cli.config import inspect_config_payload
...
def _handle_review(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.python.cli.review import handle_review

    return handle_review(args)


def _handle_export_graph(args: argparse.Namespace, *, export_kind: str) -> int:
    from vibeflow.tooling.application.python.cli.export import handle_export_graph
...
def _handle_delegate_cli(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.python.cli.delegate_cli import handle_delegate_cli

    return handle_delegate_cli(args)


def _handle_quality_check(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.python.cli.quality import handle_quality_check
...
def _inspect_workspace_config_payload(config_path: Path, workspace_path: Path) -> tuple[dict[str, object], int]:
    ...
    from vibeflow.core.findings import HealthReport
    from vibeflow.tooling.application.python.workspace_service import load_workspace_graph_for_export, validate_workspace_config_path
