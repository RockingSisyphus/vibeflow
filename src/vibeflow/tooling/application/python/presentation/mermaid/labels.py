...
from vibeflow.tooling.application.python.presentation.helpers import nodeset_for_node
...
def _resource_label(resource: Mapping[str, object], *, kind: str, show_semantics: bool) -> str:
    ...
    if show_semantics:
        ...
        if kind in {"plugin", "host_extension"}:
            fields = (
                ("targets", "provides", "dependencies")
                if kind == "host_extension"
                else ("targets", "dependencies")
            )
            for field in fields:
