...
from vibeflow.tooling.application.run_directory import parse_run_id_argument
...
def build_parser() -> argparse.ArgumentParser:
    ...
    from vibeflow.tooling.application.cli.build_command import add_build_parser

    add_build_parser(sub)

    from vibeflow.tooling.application.cli.delegate_cli import add_delegate_cli_parser

    add_delegate_cli_parser(sub, _add_runtime_options)

    from vibeflow.tooling.application.cli.review import add_review_parser

    add_review_parser(sub)

    from vibeflow.tooling.application.cli.quality import add_quality_parser
...
def main(argv: Sequence[str] | None = None) -> int:
    from vibeflow.tooling.application.cli.build_command import handle_build
...
def _handle_validate(args: argparse.Namespace) -> int:
    ...
    from vibeflow.tooling.application.reports import format_finding_text

    if getattr(args, "workspace", None):
        if args.policy:
            return _workspace_policy_arg_error()
        from vibeflow.core.findings import HealthReport
        from vibeflow.tooling.application.workspace_service import validate_workspace_config_path
...
def _handle_inspect_node(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.cli.node import inspect_node_payload
...
def _handle_inspect_config(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.cli.config import inspect_config_payload
...
def _handle_review(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.cli.review import handle_review

    return handle_review(args)


def _handle_export_graph(args: argparse.Namespace, *, export_kind: str) -> int:
    from vibeflow.tooling.application.cli.export import handle_export_graph
    ...
    try:
        if getattr(args, "workspace", None):
            if args.policy:
                return _workspace_policy_arg_error()
            from vibeflow.core.findings import HealthReport
            from vibeflow.tooling.application.workspace_service import run_workspace_checked
...
def _handle_delegate_cli(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.cli.delegate_cli import handle_delegate_cli

    return handle_delegate_cli(args)


def _handle_quality_check(args: argparse.Namespace) -> int:
    from vibeflow.tooling.application.cli.quality import handle_quality_check
...
def _inspect_workspace_config_payload(config_path: Path, workspace_path: Path) -> tuple[dict[str, object], int]:
    ...
    from vibeflow.core.findings import HealthReport
    from vibeflow.tooling.application.workspace_service import load_workspace_graph_for_export, validate_workspace_config_path
...
def _load_workspace_for_cli(path: Path):
    ...
    from vibeflow.tooling.application.workspace_service import WorkspaceConfigError, load_workspace_config
