...
from vibeflow.tooling.presentation.mermaid import MermaidRenderError
...
def _inline_fragment_svg(
    ...
    *,
    ...
    review_attributes = (
        f" data-review-kind={xml.sax.saxutils.quoteattr(fragment.review_kind)}"
        f" data-review-owner={xml.sax.saxutils.quoteattr(fragment.review_owner)}"
        f" data-review-target={xml.sax.saxutils.quoteattr(fragment.review_target)}"
    )
    return [
        f'<g class="review-inline-fragment" transform="{transform}"'
        f'{root_id_attribute}{review_attributes}>{inner}</g>'
    ]
