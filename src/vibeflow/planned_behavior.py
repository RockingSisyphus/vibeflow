...
from .ast_rules import import_aliases, is_banned_import, path_effect_call_name, qualified_call_name
from .health_types import HealthFinding
from .path_utils import is_relative_to
from .purity_types import BANNED_ATTR_CALLS, BANNED_CALL_NAMES, BANNED_IMPORT_ROOTS
...
def validate_stub_module_ref(value: str) -> str:
    ...
    if len(path.parts) < 3 or path.parts[0] != "project" or path.parts[1] != "stubs":
        return "must be under project/stubs/"
    if path.suffix != ".py":
...
def resolve_stub_module_path(stub_module: str, project_root: str | Path | None) -> Path:
    ...
    allowed_root = (root / "project" / "stubs").resolve()
