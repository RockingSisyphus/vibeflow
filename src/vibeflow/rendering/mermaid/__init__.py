...
from vibeflow.node import FLOW_KIND_DATA_STORE, FLOW_KIND_DECISION, FLOW_KIND_DOCUMENT, FLOW_KIND_IO, FLOW_KIND_PREDEFINED, FLOW_KIND_PREPARATION, FLOW_KIND_PROCESS, FLOW_KIND_TERMINAL
...
from vibeflow.rendering.style import MERMAID_MAIN_CLASS_ORDER, mermaid_class_def_lines
...
def compiled_graph_payload(graph: GraphConfig, compiled: CompiledGraph, *, resources: object | None = None) -> dict[str, object]:
    payload: dict[str, object] = {
        ...
        "nodeset_sources": [
            ...
            for nodeset in sorted(graph.nodesets.values(), key=lambda item: item.type_key)
        ],
...
class _MermaidRenderer:
    ...
    def _render_review_columns(self, graph: GraphConfig, compiled: CompiledGraph) -> str:
        from vibeflow.rendering.mermaid.review import render_review_columns
    ...
    def _nodeset_label(self, node: NodeSpec, nodeset: NodesetSpec, graph: GraphConfig) -> str:
        ...
        source_lines = _source_lines(
            nodeset.root_id or graph.root_id,
            nodeset.root_path or graph.root_path,
            nodeset.source_path or graph.source_path,
        )
        if source_lines:
            sections.append([_section_label("source"), *source_lines])
