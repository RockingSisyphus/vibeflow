"""Python ``base_lib`` source, dependency, and project-file analysis."""
...
from vibeflow.targets.python.project.base_lib_types import (
...
FORBIDDEN_PROJECT_IMPORT_PARTS = {
    "boundary", "boundaries", "nodes", "plugin", "plugins", "runtime",
}


def analyze_base_lib_source(
    source_text: str,
    *,
    module: str,
    path: str,
    ...
    """Analyze one already-loaded base_lib module."""
    ...
    findings.extend(_size_findings(path, module, source_text, policy=policy))
...
def build_base_lib_scan_report(
    *,
    roots: tuple[str, ...],
    modules: tuple[BaseLibModuleReport, ...],
) -> BaseLibScanReport:
    """Build dependency edges and closure findings from module reports."""

    reports = {report.module: report for report in modules}
    dependency_edges = tuple(
        (report.module, resolved)
        for report in modules
        for imported in report.imports
        for resolved in (_resolve_base_lib_import(imported, reports),)
        if resolved
    )
    findings = [finding for report in modules for finding in report.findings]
    findings.extend(_dependency_closure_findings(reports, dependency_edges))
...
def summarize_base_lib_dependency_chain(
    imported_modules: tuple[str, ...],
    report: BaseLibScanReport,
) -> BaseLibDependencySummary:
...
def base_lib_imports_from_source(source_text: str) -> tuple[str, ...]:
    """Return base_lib imports from already-loaded Python source."""
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            imports.update(
                alias.name for alias in node.names if _is_base_lib_module(alias.name)
            )
        elif (
            isinstance(node, ast.ImportFrom)
            and node.module
            and _is_base_lib_module(node.module)
        ):
...
def scan_base_lib(
    ...
    *,
    ...
    """Discover and analyze Python base_lib files for one project root."""
    ...
    return build_base_lib_scan_report(
...
def node_base_lib_imports(node_cls: type) -> tuple[str, ...]:
    """Read a registered Python node and return its base_lib imports."""

    path = Path(inspect.getsourcefile(node_cls) or "")
    if not path.exists():
        return ()
    return base_lib_imports_from_source(path.read_text(encoding="utf-8"))
...
class _BaseLibAstScanner(ast.NodeVisitor):
    def __init__(self, *, path: str, module: str, policy: PurityPolicy) -> None:
        self.path = path
        self.module = module
        self.policy = policy
        ...
        for stmt in node.body:
            ...
            if kind in {"import", "definition"}:
                ...
                self._add(
                    ...
                    "base_lib top level may only contain imports, definitions, immutable constants, and docstrings",
                    stmt,
                )
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if process_argv_import_is_forbidden(
            module,
            {alias.name for alias in node.names},
        ):
            self._add(
                "BASE_LIB.BANNED_IMPORT",
                "base_lib must not import process sys.argv",
                node,
            )
        self._check_import_node(node)
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if violation_code:
            self._add(
                ...
                f"base_lib banned side-effect call: {forbidden}",
                node,
            )
    ...
    def visit_Raise(self, node: ast.Raise) -> None:
        if not isinstance(node.exc, ast.Call):
            ...
            if reference and system_exit_is_forbidden(EFFECT_SCOPE_NONE):
                self._add(
                    "BASE_LIB.SIDE_EFFECT_CALL",
                    f"base_lib banned process exit: {reference}",
                    node,
                )
        self.generic_visit(node)
    ...
    def visit_Global(self, node: ast.Global) -> None:
        self._add(
            ...
            "base_lib must not use global mutation",
            node,
        )
    ...
    def visit_Nonlocal(self, node: ast.Nonlocal) -> None:
        self._add(
            "BASE_LIB.GLOBAL_STATE",
            "base_lib must not use nonlocal mutation",
            node,
        )
    ...
    def _visit_module_assignment(self, node: ast.Assign | ast.AnnAssign) -> None:
        ...
        if not module_assignment_is_allowed(node):
            self._add(
                "BASE_LIB.GLOBAL_STATE",
                "base_lib must not hold mutable module-level state",
                node,
            )
        self.visit(node)
    ...
    def _check_import(self, module: str, node: ast.AST) -> None:
        ...
        if any(
            part in FORBIDDEN_PROJECT_IMPORT_PARTS for part in module.split(".")
        ):
            self._add(
                "BASE_LIB.FORBIDDEN_PROJECT_IMPORT",
                "base_lib must not import node, plugin, boundary, runtime, or "
                f"side-effect layer: {module}",
                node,
            )
            return
        if module_matches(module, self.policy.banned_base_lib_modules):
            self._add(
                "BASE_LIB.BANNED_MODULE",
                f"base_lib module is banned by policy: {module}",
                node,
            )
            return
        if import_violation_code(
            ...
            self._add(
                "BASE_LIB.BANNED_IMPORT",
                f"base_lib banned import: {module}",
                node,
            )
...
def _size_findings(
    ...
    *,
    ...
    if lines > policy.max_source_lines:
        findings.append(
            _finding(
                "BASE_LIB.SOURCE.MAX_LINES",
                f"base_lib file has {lines} lines > {policy.max_source_lines}",
                path=path,
                module=module,
                details={"lines": lines, "limit": policy.max_source_lines},
            )
        )
    if bytes_ > policy.max_source_bytes:
        findings.append(
            _finding(
                "BASE_LIB.SOURCE.MAX_BYTES",
                f"base_lib file has {bytes_} bytes > {policy.max_source_bytes}",
                path=path,
                module=module,
                details={"bytes": bytes_, "limit": policy.max_source_bytes},
            )
        )
...
def _complexity_findings(
    ...
    *,
    ...
    checks = (
        (
            "BASE_LIB.COMPLEXITY.MAX_FUNCTIONS",
            policy.max_functions,
            counter.function_count,
            "function_count",
        ),
        (
            "BASE_LIB.COMPLEXITY.MAX_BRANCHES",
            policy.max_branches,
            counter.branch_count,
            "branch_count",
        ),
        (
            "BASE_LIB.COMPLEXITY.MAX_NESTING_DEPTH",
    ...
    for rule_id, limit, actual, key in checks:
        if limit is not None and actual > limit:
            findings.append(
                _finding(
                    rule_id,
                    f"base_lib {key} is {actual} > policy limit {limit}",
                    path=path,
                    module=module,
                    details={key: actual, "limit": limit},
                )
            )
    return tuple(findings)
        ...
        if target in bad_modules:
            ...
            findings.append(
                _finding(
                    ...
                    "base_lib module depends on unhealthy base_lib module: "
                    f"{source} -> {target}",
                    path=report.path,
                    module=source,
...
def _resolve_base_lib_import(
    module: str,
    reports: Mapping[str, BaseLibModuleReport],
) -> str:
    ...
    for candidate in reports:
        if (
            module == candidate
            or candidate.endswith(f".{module}")
            or module.endswith(f".{candidate}")
        ):
            return candidate
    return ""
...
def _is_base_lib_info_assignment(node: ast.Assign | ast.AnnAssign) -> bool:
    ...
    if not any(
        isinstance(target, ast.Name) and target.id == "BASE_LIB_INFO"
        for target in targets
    ):
        return False
    return isinstance(node.value, (ast.Call, ast.Dict))
...
__all__ = [
    "analyze_base_lib_source", "base_lib_imports_from_source",
    "build_base_lib_scan_report", "node_base_lib_imports", "scan_base_lib",
