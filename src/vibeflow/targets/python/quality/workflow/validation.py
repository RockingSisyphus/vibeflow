...
from vibeflow.targets.python.quality.workflow.flow import append_data_contract_warnings, append_flowchart_health, append_join_policy_health
...
from vibeflow.targets.python.quality.workflow.report import _build_health_report
...
if TYPE_CHECKING:
    from vibeflow.core.compiler import CompiledGraph, GraphCompileError
...
def validate_graph_health(
    ...
    *,
    ...
    node_quality_facts: Mapping[str, PythonNodeQualityFacts] | None = None,
) -> HealthReport:
    ...
    from vibeflow.targets.python.quality.workflow.nodesets import nodeset_depth_error_report
...
def _validate_graph_nodes(
    ...
    node_quality_facts: Mapping[str, PythonNodeQualityFacts],
    state: _HealthValidationState,
) -> None:
    for spec in graph.nodes:
        ...
        _validate_graph_node(
            ...
            plugin_registry,
            purity_policy,
            node_quality_facts.get(spec.type_used),
            state,
        )
...
def _validate_graph_node(
    ...
    plugin_registry: PluginRegistry | None,
    purity_policy: PurityPolicy | None,
    node_quality_facts: PythonNodeQualityFacts | None,
    state: _HealthValidationState,
) -> None:
    ...
    try:
        ...
        return
    from vibeflow.targets.python.quality.source_analysis import collect_node_metrics
    ...
    _append_base_lib_health(spec.id, node_quality_facts, purity_policy, state)
...
def _append_base_lib_health(
    ...
    from vibeflow.targets.python.project.base_lib import summarize_base_lib_dependency_chain
    from vibeflow.targets.python.quality.workflow.base_lib import append_dependency_chain_findings, matching_unhealthy_base_module

    if node_quality_facts is None or node_quality_facts.base_lib_report is None:
        return
    base_report = node_quality_facts.base_lib_report
    imported_modules = node_quality_facts.base_lib_imports
...
def _append_base_report_findings(findings, state: _HealthValidationState, *, relevant_modules: set[str]) -> None:
    from vibeflow.targets.python.quality.workflow.base_lib import base_lib_finding_to_health
...
def _append_indirect_base_lib_findings(node_name: str, imported_modules: tuple[str, ...], state: _HealthValidationState) -> None:
    from vibeflow.targets.python.quality.workflow.base_lib import matching_unhealthy_base_module
...
def _append_node_purity_findings(
    ...
    from vibeflow.targets.python.quality.source_analysis import validate_node_class
    from vibeflow.targets.python.quality.workflow.plugins import append_finding_by_severity
...
def _append_node_config_health(graph: GraphConfig, registry: NodeRegistry, state: _HealthValidationState, *, global_config: Mapping[str, object] | None) -> None:
    from vibeflow.targets.python.quality.workflow.node_config import validate_node_config_health
...
def _append_registry_namespace_smells(registry: NodeRegistry, state: _HealthValidationState) -> None:
    from vibeflow.targets.python.quality.workflow.registry import registry_namespace_findings
...
def _append_nodeset_health(graph: GraphConfig, registry: NodeRegistry | None, state: _HealthValidationState) -> None:
    from vibeflow.targets.python.quality.workflow.nodesets import append_nodeset_finding, validate_nodesets
...
def _validate_plugin_schema_extensions(
    ...
    from vibeflow.targets.python.project.plugins import plugin_error
...
__all__ = ["_HealthValidationState", "validate_graph_health"]
