...
class IoInputNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('io.result'),), provides=(PROV('io.result.input', 'io.result'),), input_semantics={'io.result': ('外部系统或测试初始上下文提供的数值结果。',)}, output_semantics={'io.result.input': ('pass-through io.result envelope from pipeline input',)}, examples=({'inputs': {'io.result': {'key': 'io.result', 'type': 'io.result', 'value': 1, 'source_node': 'example'}}, 'params': {}},))
...
class ConstantNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('value.in'),), output_semantics={'value.in': ('configured numeric value',)}, examples=({'inputs': {}, 'params': {'value': 2}},))
...
class MultiplyNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.out'),), provides=(PROV('value.final'),), input_semantics={'value.out': ('value to multiply',)}, output_semantics={'value.final': ('final numeric value',)}, examples=({'inputs': {'value.out': {'key': 'value.out', 'type': 'value.out', 'value': 5, 'source_node': 'example'}}, 'params': {'factor': 2}},))
...
class BranchLeftNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('branch.left'),), output_semantics={'branch.left': ('left branch value',)}, examples=({'inputs': {}, 'params': {'value': 4}},))
...
class BranchRightNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('branch.right'),), output_semantics={'branch.right': ('right branch value',)}, examples=({'inputs': {}, 'params': {'value': 6}},))
...
class SumPairNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('branch.left'), REQ('branch.right')), provides=(PROV('value.in'),), input_semantics={'branch.left': ('left value',), 'branch.right': ('right value',)}, output_semantics={'value.in': ('summed value',)}, examples=({'inputs': {'branch.left': {'key': 'branch.left', 'type': 'branch.left', 'value': 4, 'source_node': 'example'}, 'branch.right': {'key': 'branch.right', 'type': 'branch.right', 'value': 6, 'source_node': 'example'}}, 'params': {}},))
...
class AddThreeNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.in'),), provides=(PROV('value.out'),), input_semantics={'value.in': ('input numeric value',)}, output_semantics={'value.out': ('output numeric value',)}, examples=({'inputs': {'value.in': {'key': 'value.in', 'type': 'value.in', 'value': 2, 'source_node': 'example'}}, 'params': {}},))
...
class AddOutNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.out'),), provides=(PROV('value.final'),), input_semantics={'value.out': ('input numeric value',)}, output_semantics={'value.final': ('final numeric value',)}, examples=({'inputs': {'value.out': {'key': 'value.out', 'type': 'value.out', 'value': 2, 'source_node': 'example'}}, 'params': {'delta': 1}},))

    def run_pure(self, inputs, params):
        return {'value.final': add(VALUE(inputs, 'value.out'), params.get('delta', 1))}

class AddFinalToDeadNode:
    NODE_INFO = NodeInfo(type_key='sandbox.add_final_to_dead', display_name='Add Final To Dead', category='sandbox', description='Extends the deliberately dead decision branch using a distinct, type-owned contract.', version='0.1.0', flow_kind='process')
    CONTRACT = NodeContract(requires=(REQ('value.final'),), provides=(PROV('value.dead', 'value.final'),), input_semantics={'value.final': ('value from the first dead-branch step',)}, output_semantics={'value.dead': ('continued value on the dead branch',)}, examples=({'inputs': {'value.final': {'key': 'value.final', 'type': 'value.final', 'value': 3, 'source_node': 'example'}}, 'params': {'delta': 1}},))

    def run_pure(self, inputs, params):
        return {'value.dead': add(VALUE(inputs, 'value.final'), params.get('delta', 1))}
...
class ResourceScaleNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.resource'),), provides=(PROV('calc.scaled'),), input_semantics={'calc.resource': ('resource arithmetic value to scale',)}, output_semantics={'calc.scaled': ('calc.resource multiplied by factor',)}, examples=({'inputs': {'calc.resource': {'key': 'calc.resource', 'type': 'calc.resource', 'value': 50, 'source_node': 'example'}}, 'params': {'factor': 4}},))
