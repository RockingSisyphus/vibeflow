...
class SemanticScaleNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.sum'),), provides=(PROV('calc.scaled'),), input_semantics={'calc.sum': ('numeric sum to multiply',)}, output_semantics={'calc.scaled': ('calc.sum multiplied by factor',)}, examples=({'inputs': {'calc.sum': {'key': 'calc.sum', 'type': 'calc.sum', 'value': 7, 'source_node': 'example'}}, 'params': {'factor': 3}},))
...
class SemanticUseScaledNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.scaled'),), provides=(PROV('calc.branch'),), input_semantics={'calc.scaled': ('scaled arithmetic value',)}, output_semantics={'calc.branch': ('branch value copied from calc.scaled',)}, examples=({'inputs': {'calc.scaled': {'key': 'calc.scaled', 'type': 'calc.scaled', 'value': 21, 'source_node': 'example'}}, 'params': {}},))
...
class SemanticCompareGtNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.c'), REQ('calc.d')), provides=(PROV('route.branch'),), input_semantics={'calc.c': ('left comparison value',), 'calc.d': ('right comparison value',)}, output_semantics={'route.branch': ('left when calc.c > calc.d, otherwise right',)}, examples=({'inputs': {'calc.c': {'key': 'calc.c', 'type': 'calc.c', 'value': 9, 'source_node': 'example'}, 'calc.d': {'key': 'calc.d', 'type': 'calc.d', 'value': 4, 'source_node': 'example'}}, 'params': {}}, {'inputs': {'calc.c': {'key': 'calc.c', 'type': 'calc.c', 'value': 1, 'source_node': 'example'}, 'calc.d': {'key': 'calc.d', 'type': 'calc.d', 'value': 4, 'source_node': 'example'}}, 'params': {}}))
...
class SemanticLeftAdjustNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.scaled'),), provides=(PROV('calc.left_branch', 'branch_result'),), input_semantics={'calc.scaled': ('scaled arithmetic value selected for the left branch',)}, output_semantics={'calc.left_branch': ('left branch value after adding bonus',)}, examples=({'inputs': {'calc.scaled': {'key': 'calc.scaled', 'type': 'calc.scaled', 'value': 21, 'source_node': 'example'}}, 'params': {'bonus': 10}},))
...
class SemanticRightAdjustNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.scaled'),), provides=(PROV('calc.right_branch', 'branch_result'),), input_semantics={'calc.scaled': ('scaled arithmetic value selected for the right branch',)}, output_semantics={'calc.right_branch': ('right branch value after subtracting penalty',)}, examples=({'inputs': {'calc.scaled': {'key': 'calc.scaled', 'type': 'calc.scaled', 'value': 21, 'source_node': 'example'}}, 'params': {'penalty': 6}},))

    def run_pure(self, inputs, params):
        return {'calc.right_branch': VALUE(inputs, 'calc.scaled') - params.get('penalty', 0)}

class SemanticLeftKeyAdjustNode:
    NODE_INFO = NodeInfo(type_key='semantic.left_key_adjust', display_name='Semantic Left Key Adjust', category='semantic', description='Adds a configured bonus and exposes the result with the calc.left_branch data type.', version='0.1.0', flow_kind='process')
    CONTRACT = NodeContract(requires=(REQ('calc.scaled'),), provides=(PROV('calc.left_branch'),), input_semantics={'calc.scaled': ('scaled arithmetic value selected for the left branch',)}, output_semantics={'calc.left_branch': ('left branch value after adding bonus',)})

    def run_pure(self, inputs, params):
        return {'calc.left_branch': VALUE(inputs, 'calc.scaled') + params.get('bonus', 0)}

class SemanticRightKeyAdjustNode:
    NODE_INFO = NodeInfo(type_key='semantic.right_key_adjust', display_name='Semantic Right Key Adjust', category='semantic', description='Subtracts a configured penalty and exposes the result with the calc.right_branch data type.', version='0.1.0', flow_kind='process')
    CONTRACT = NodeContract(requires=(REQ('calc.scaled'),), provides=(PROV('calc.right_branch'),), input_semantics={'calc.scaled': ('scaled arithmetic value selected for the right branch',)}, output_semantics={'calc.right_branch': ('right branch value after subtracting penalty',)})
...
class SemanticBranchTypeConsumerNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('branch_result'),), provides=(PROV('branch.final', 'final_result'),), input_semantics={'branch_result': ('one selected branch result envelope',)}, output_semantics={'branch.final': ('source-aware final branch value',)}, examples=({'inputs': {'branch_result': {'key': 'calc.left_branch', 'type': 'branch_result', 'value': 31, 'source_node': 'example'}}, 'params': {}},))
...
class SemanticJoinPassthroughNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.in'),), provides=(PROV('value.out'),), input_semantics={'value.in': ('value selected by a safe join',)}, output_semantics={'value.out': ('selected value after the join',)}, examples=({'inputs': {'value.in': {'key': 'value.left', 'type': 'value.in', 'value': 3, 'source_node': 'example'}}, 'params': {}},))
...
class SemanticInactiveRouteNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('flow.route'),), output_semantics={'flow.route': ('false route condition',)}, examples=({'inputs': {}, 'params': {}},))
...
class SemanticConditionalValueNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('value.alt', 'value.in'), PROV('flow.use_alt')), output_semantics={'value.alt': ('conditional value.in provider',), 'flow.use_alt': ('true route condition',)}, examples=({'inputs': {}, 'params': {'value': 7}},))
...
class SemanticLeftValueNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('value.left', 'value.in'),), output_semantics={'value.left': ('left value provider',)}, examples=({'inputs': {}, 'params': {'value': 1}},))
...
class SemanticRightValueNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('value.right', 'value.in'),), output_semantics={'value.right': ('right value provider',)}, examples=({'inputs': {}, 'params': {'value': 2}},))
...
class SemanticOtherValueNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('other.in'),), output_semantics={'other.in': ('secondary join input',)}, examples=({'inputs': {}, 'params': {'value': 5}},))
...
class SemanticTwoInputJoinNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.in'), REQ('other.in')), provides=(PROV('value.out'),), input_semantics={'value.in': ('primary join input',), 'other.in': ('secondary join input',)}, output_semantics={'value.out': ('sum of both join inputs',)}, examples=({'inputs': {'value.in': {'key': 'value.left', 'type': 'value.in', 'value': 3, 'source_node': 'example'}, 'other.in': {'key': 'other.in', 'type': 'other.in', 'value': 5, 'source_node': 'example'}}, 'params': {}},))
...
class SemanticFinalizeNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('calc.branch'),), provides=(PROV('calc.final'),), input_semantics={'calc.branch': ('branch or promoted scaled value to finalize',)}, output_semantics={'calc.final': ('calc.branch plus offset',)}, examples=({'inputs': {'calc.branch': {'key': 'calc.branch', 'type': 'calc.branch', 'value': 31, 'source_node': 'example'}}, 'params': {'offset': 1}},))
...
class SemanticIncrementUntilNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('loop.current'),), provides=(PROV('loop.next'),), input_semantics={'loop.current': ('current loop value before increment',)}, output_semantics={'loop.next': ('loop.current plus step',)}, examples=({'inputs': {'loop.current': {'key': 'loop.current', 'type': 'loop.current', 'value': 1, 'source_node': 'example'}}, 'params': {'step': 2}},))
...
class SemanticLoopDoneNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('loop.next'),), provides=(PROV('loop.done'),), input_semantics={'loop.next': ('candidate loop value after increment',)}, output_semantics={'loop.done': ('true when loop.next is greater than or equal to target',)}, examples=({'inputs': {'loop.next': {'key': 'loop.next', 'type': 'loop.next', 'value': 5, 'source_node': 'example'}}, 'params': {'target': 7}}, {'inputs': {'loop.next': {'key': 'loop.next', 'type': 'loop.next', 'value': 7, 'source_node': 'example'}}, 'params': {'target': 7}}))
...
class SemanticLoopDoneValueNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('loop.next'),), provides=(PROV('loop.done'),), input_semantics={'loop.next': ('candidate loop value after increment',)}, output_semantics={'loop.done': ('true when loop.next is greater than or equal to target',)}, examples=({'inputs': {'loop.next': {'key': 'loop.next', 'type': 'loop.next', 'value': 5, 'source_node': 'example'}}, 'params': {'target': 7}},))
...
class SemanticNestedLoopSeedNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('outer.current'), PROV('total.current')), output_semantics={'outer.current': ('initial outer loop counter',), 'total.current': ('initial nested-loop accumulator',)}, examples=({'inputs': {}, 'params': {'outer_start': 0, 'total_start': 0}},))
...
class SemanticOuterAdvanceNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('outer.inner'), REQ('total.inner')), provides=(PROV('outer.current.next', 'outer.current'), PROV('total.current.next', 'total.current'), PROV('outer.done')), input_semantics={'outer.inner': ('outer loop counter before advance',), 'total.inner': ('running total after the inner loop',)}, output_semantics={'outer.current.next': ('next outer loop counter',), 'total.current.next': ('running total carried to the next outer iteration',), 'outer.done': ('true when the next outer counter reaches the configured limit',)}, examples=({'inputs': {'outer.inner': {'key': 'outer.inner', 'type': 'outer.inner', 'value': 1, 'source_node': 'example'}, 'total.inner': {'key': 'total.inner', 'type': 'total.inner', 'value': 8, 'source_node': 'example'}}, 'params': {'outer_step': 1, 'outer_limit': 2}},))
...
class SemanticCopyNextNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('loop.next'),), provides=(PROV('loop.current.copy', 'loop.current'),), input_semantics={'loop.next': ('latest loop value to reuse',)}, output_semantics={'loop.current.copy': ('next loop iteration input',)}, examples=({'inputs': {'loop.next': {'key': 'loop.next', 'type': 'loop.next', 'value': 3, 'source_node': 'example'}}, 'params': {}},))
...
class SemanticSlowAsyncValueNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('async.value'),), output_semantics={'async.value': ('slow async value that should remain unconsumed',)}, examples=({'inputs': {}, 'params': {'value': 42}},))
