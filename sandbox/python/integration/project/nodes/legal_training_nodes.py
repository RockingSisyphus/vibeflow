...
class OrdinaryCallbackWarningNode:
    NODE_INFO = NodeInfo('sandbox.ordinary_callback', 'Ordinary Callback Warning', 'training', 'Deliberately invokes an envelope callback from an ordinary node so the advisory runtime-dispatch warning remains executable.', '0.1.0', 'process')
    CONTRACT = NodeContract(requires=(REQ('callback.in'),), provides=(PROV('value.out'),), input_semantics={'callback.in': ('runtime callback or example scalar',)}, output_semantics={'value.out': ('callback result',)}, examples=({'inputs': {'callback.in': {'key': 'callback.in', 'type': 'callback.in', 'value': 7, 'source_node': 'example'}}, 'params': {}},))

    def run_pure(self, inputs, params):
        callback = VALUE(inputs, 'callback.in')
        return {'value.out': callback() if callable(callback) else callback}
...
class ForwardLossNode:
    NODE_INFO = NodeInfo('sandbox.forward_loss', 'Forward Loss', 'training', 'Computes a loss through a runtime-provided model callback.', '0.1.0', 'global_state')
    CONTRACT = NodeContract(requires=(REQ('train.model'), REQ('train.batch')), provides=(PROV('train.loss'),), input_semantics={'train.model': ('model object',), 'train.batch': ('batch object',)}, output_semantics={'train.loss': ('numeric loss',)}, examples=({'inputs': {'train.model': {'key': 'train.model', 'type': 'train.model', 'value': {}, 'source_node': 'example'}, 'train.batch': {'key': 'train.batch', 'type': 'train.batch', 'value': {}, 'source_node': 'example'}}, 'params': {}},))
...
class BackwardGradNode:
    NODE_INFO = NodeInfo('sandbox.backward_grad', 'Backward Grad', 'training', 'Computes a gradient through a runtime-provided model callback.', '0.1.0', 'global_state')
    CONTRACT = NodeContract(requires=(REQ('train.model'), REQ('train.loss')), provides=(PROV('train.grad'),), input_semantics={'train.model': ('model object',), 'train.loss': ('numeric loss',)}, output_semantics={'train.grad': ('numeric gradient',)}, examples=({'inputs': {'train.model': {'key': 'train.model', 'type': 'train.model', 'value': {}, 'source_node': 'example'}, 'train.loss': {'key': 'train.loss', 'type': 'train.loss', 'value': 1, 'source_node': 'example'}}, 'params': {}},))
...
class TrainingMetricsNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('train.model_after'), REQ('train.loss')), provides=(PROV('train.metrics'),), input_semantics={'train.model_after': ('model object',), 'train.loss': ('numeric loss',)}, output_semantics={'train.metrics': ('metrics object containing non-JSON values',)}, examples=({'inputs': {'train.model_after': {'key': 'train.model_after', 'type': 'train.model_after', 'value': {}, 'source_node': 'example'}, 'train.loss': {'key': 'train.loss', 'type': 'train.loss', 'value': 1, 'source_node': 'example'}}, 'params': {}},))
...
class TrainingBatchStepNode:
    NODE_INFO = NodeInfo('sandbox.training_batch_step', 'Training Batch Step', 'training', 'Runs runtime-provided model and optimizer callbacks for one training batch.', '0.1.0', 'global_state')
...
class BatchMetricsNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('train.batch'),), provides=(PROV('train.metrics'),), input_semantics={'train.batch': ('batch object',)}, output_semantics={'train.metrics': ('non-JSON metrics',)}, examples=({'inputs': {'train.batch': {'key': 'train.batch', 'type': 'train.batch', 'value': {}, 'source_node': 'example'}}, 'params': {}},))
