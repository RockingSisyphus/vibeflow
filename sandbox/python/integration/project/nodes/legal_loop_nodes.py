...
class IncrementNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.in'),), provides=(PROV('value.next'),), input_semantics={'value.in': ('current value',)}, output_semantics={'value.next': ('next value',)}, examples=({'inputs': {'value.in': {'key': 'value.in', 'type': 'value.in', 'value': 1, 'source_node': 'example'}}, 'params': {}},))
...
class CopyBackNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.next'), REQ('loop.done')), provides=(PROV('value.in.copy', 'value.in'),), input_semantics={'value.next': ('next value',), 'loop.done': ('whether the loop should stop',)}, output_semantics={'value.in.copy': ('loop current value for the next iteration',)}, examples=({'inputs': {'value.next': {'key': 'value.next', 'type': 'value.next', 'value': 2, 'source_node': 'example'}, 'loop.done': {'key': 'loop.done', 'type': 'loop.done', 'value': False, 'source_node': 'example'}}, 'params': {}},))
...
class DoneCheckNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.next'),), provides=(PROV('loop.done'),), input_semantics={'value.next': ('next value',)}, output_semantics={'loop.done': ('whether the loop should stop',)}, examples=({'inputs': {'value.next': {'key': 'value.next', 'type': 'value.next', 'value': 3, 'source_node': 'example'}}, 'params': {'target': 3}},))
