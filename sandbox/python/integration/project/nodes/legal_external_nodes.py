...
class EffectRequestNode:
    ...
    CONTRACT = NodeContract(provides=(PROV('effects.request'),), output_semantics={'effects.request': ('structured request for an external effect',)}, examples=({'inputs': {}, 'params': {'value': 5}},))
...
class IoResultAddNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('io.result'),), provides=(PROV('value.final'),), input_semantics={'io.result': ('external numeric result',)}, output_semantics={'value.final': ('final numeric value',)}, examples=({'inputs': {'io.result': {'key': 'io.result', 'type': 'io.result', 'value': 7, 'source_node': 'example'}}, 'params': {'delta': 1}},))
...
class IoResultInputNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('io.result'),), provides=(PROV('value.in'),), input_semantics={'io.result': ('external numeric result',)}, output_semantics={'value.in': ('numeric input for downstream flow',)}, examples=({'inputs': {'io.result': {'key': 'io.result', 'type': 'io.result', 'value': 7, 'source_node': 'example'}}, 'params': {'delta': 1}},))
