...
The easiest way is to use [pre-configured model blocks](/reference#models) in your configuration:
...
Continue may use a higher default context length than other tools. Reduce `contextLength` in your config (e.g., to 2048), or try a smaller model. See [Ollama provider troubleshooting](/customize/model-providers/top-level/ollama#troubleshooting) for details.
...
**Problem**: `ollama pull deepseek-r1` installs `:latest` but model block expects `:32b`
