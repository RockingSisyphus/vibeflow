...
def test_workspace_cross_root_nodeset_validate_run_and_export(tmp_path) -> None:
    ...
    config_path.write_text(
        json.dumps(
            {
                "nodeset_imports": [{"root": "framework", "path": "configs/nodesets/add_one.jsonc"}],
                "pipeline": {
                    "nodes": [
    ...
    assert report.info["workspace"]["roots"][0]["id"] == "framework"
    assert report.info["nodeset_imports"][0]["root_id"] == "framework"
    ...
    assert graph.nodesets["framework.add_one"].root_id == "framework"
    ...
    assert "root: framework" in mermaid
...
def test_workspace_nodeset_depth_limit_uses_workflow_root_config(tmp_path) -> None:
    ...
    assert framework_depth.root_id == "framework"
...
def test_workspace_registry_conflicts_report_roots(tmp_path) -> None:
    ...
    assert "framework" in conflict.value.message
...
""".strip(),
    ...
    (project_root / "registry.py").write_text(
        """
from vibeflow.targets.python.project import BaseLibRegistry, NodeRegistry
from tests.fixtures.support import strict_support_runtime_nodes as support
...
def _workspace_fixture(tmp_path: Path, *, write_workspace: bool = True) -> tuple[Path, Path, Path]:
    ...
    framework_root = repo / "framework"
    project_root.mkdir(parents=True)
    framework_root.mkdir(parents=True)
    workspace_path = repo / "vibeflow_config.jsonc"
    if write_workspace:
        workspace_path.write_text(
            json.dumps({"policy": {}, "roots": [{"id": "framework", "path": "framework"}, {"id": "project", "path": "project"}]}),
            encoding="utf-8",
        )
...
def _append_resource_registry(
    ...
    *,
    ...
    lines = [
        "",
        "from vibeflow.targets.python.project import BaseLibRegistry, PluginResourceRegistry",
        "",
        "def build_base_lib_registry():",
        "    registry = BaseLibRegistry()",
    ]
