...
from vibeflow.aot.builder import BuildResult
from vibeflow.aot.model import normalize_workflow_plan
...
from vibeflow.aot.toolchain import ToolchainInfo
...
def test_prepare_project_build_rejects_python_runtime_plugin(
    ...
    monkeypatch.setattr(
        "vibeflow.aot.project_build._project_plugins",
...
def test_build_project_aot_forwards_prepared_request(
    ...
    monkeypatch.setattr("vibeflow.aot.project_build.build_aot", fake_build)
    ...
    assert forwarded.import_policy["node_base_libs"] == {
        "demo.add": ["demo.math"],
        "demo.start": [],
    }
