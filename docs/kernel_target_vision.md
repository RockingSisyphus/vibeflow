...
`topology-kernel` 服务于人机协同开发，尤其是大量依赖 LLM 编写、修改和维护代码的项目。它的目标不是让程序更容易随意扩展，而是把架构纪律变成可执行的硬约束。
...
内核要把这些风险前移到配置、契约、编译、健康检查和运行时中处理。

## 核心目标

`topology-kernel` 是可迁移、可复用的严格标准流程图内核。业务开发者只写小型 node、纯 `base_lib` helper、必要插件和 JSONC 拓扑配置。
...
- `requires` / `provides` 是数据契约，不是控制流推导来源。
...
```jsonc
{
  "pipeline": {
    "nodes": [
      ...
      {"name": "prepare", "type": "demo.prepare", "requires": ["value.in"], "provides": ["value.ready"]},
      {"name": "route", "type": "demo.route", "requires": ["value.ready"], "provides": ["flow.route"]},
      {"name": "end", "type": "demo.end", "requires": ["value.ready"]}
    ],
    "edges": [
      ["start", "prepare"],
      ["prepare", "route"],
