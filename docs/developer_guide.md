...
`flow_kind` 只决定流程语义、图形和适用校验，不是能力或权限声明。`io` 只适配已传入的外部表示或形成输出对象，`data_store` 只形成存储请求/引用，`document` 只形成文档对象/请求。这些类别都不授予读写文件、访问网络/数据库、控制浏览器或启动进程的能力；真实副作用由 VibeFlow 调用方或内核外部 adapter 完成。
...
`external=True` 是“实现由第三方或外部维护”的信任/检查边界，只跳过内部源码质量检查，例如复杂度、导入链、源码规模。它不是 IO 权限或 purity 绕过开关，也不会跳过：
