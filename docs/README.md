...
VibeFlow 0.12.0 使用一个语言无关 Core、一个公共 Block Compiler，以及彼此隔离的 Python 和 JavaScript Target。每个 workspace root 必须用 `project_target` 明确选择一种 Target；单次 workflow 不混用两种语言实现。
...
正式目录写入 `../dist/vibeflow-distribution/`，确定性归档写入 `../archive/vibeflow-distribution-0.12.0.zip`。临时验证可使用 `--output-dir` 和 `--archive-dir`。
