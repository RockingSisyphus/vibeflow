...
- Config node visual metadata (`display_name`, `description`, `style`, `similar_to`), first-class loop nodes, safe OR join / `join_policy`, SVG color rules, detailed health warning `details`, and nested runtime trace fields are documented in `developer_guide.md`.
