...
- Config node visual metadata (`display_name`, `description`, `style`, `similar_to`), symbol-table nodeset parsing and forward references, first-class loop nodes, safe OR join / `join_policy`, SVG color rules, actionable and aggregated health/quality `details`, config parse tracing, and nested runtime trace fields are documented in `developer_guide.md` and `kernel_development_guide.md`.
