...
- [ ] 普通 node 无法直接产生副作用。
- [ ] 普通 node 无法直接调用或导入其他 node。
- [ ] `base_lib` 无法成为隐藏副作用或巨型逻辑的逃逸口。
- [ ] 所有拓扑关系只能由 JSONC 配置显式组织。
- [ ] 所有环路必须显式声明且有执行上限。
- [ ] nodeset 可以承载复杂功能，但不会泄漏内部 key 或递归引用。
- [ ] 运行入口无法绕过强制健康检查。
- [ ] 健康检查失败会拒绝运行。
- [ ] 健康报告能定位失败对象、规则来源、失败层级和建议修复类型。
- [ ] Mermaid、编译图、健康报告和运行 trace 能共同解释程序结构。
- [ ] 插件可以扩展或收紧治理，但不能隐式绕过绝对规则。
- [ ] 每次运行都有可审计、可复现的运行产物。
...
- [ ] 支持对任意项目目录运行代码质量检查，不要求项目采用 node / nodeset 架构。
- [ ] 检查每个代码文件是否超过最大行数，默认阈值为 500 行。
- [ ] 检查每个代码文件的字节数、函数数量、分支数量和最大嵌套深度。
- [ ] 检查单个函数是否过长、分支过多或嵌套过深。
- [ ] 扫描 Python import 图，发现过长依赖链条。
- [ ] 发现循环 import 或明显的双向模块依赖。
- [ ] 检查是否存在过宽模块，例如单文件承担过多函数、类或公共 API。
- [ ] 检查是否存在明显的重复逻辑信号，例如相似函数名、相似 AST 指纹或重复转换逻辑。
- [ ] 检查是否存在隐藏副作用高风险代码，例如文件、网络、数据库、外部进程和环境变量访问。
- [ ] 输出独立的质量报告 JSON，包含文件、函数、依赖链、规则编号、严重程度和建议修复类型。
- [ ] 支持把检查结果以人类可读摘要输出到 CLI。
- [ ] 为该工具增加自检入口，用于检查 `topology-kernel` 仓库自身。
- [ ] 在 CI 或发布前流程中预留接入点，确保内核自身也接受同等代码质量约束。
