...
class AddNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('value.in'),), provides=(PROV('semantic.value'),), input_semantics={'value.in': ('input numeric value',)}, output_semantics={'semantic.value': ('internal arithmetic result',)}, examples=({'inputs': {'value.in': {'key': 'value.in', 'type': 'value.in', 'value': 2, 'source_node': 'example'}}, 'params': {'delta': 3}},))
...
class OutputNode:
    ...
    CONTRACT = NodeContract(requires=(REQ('semantic.value'),), provides=(PROV('response.value'),), input_semantics={'semantic.value': ('internal arithmetic result',)}, output_semantics={'response.value': ('external numeric response',)}, examples=({'inputs': {'semantic.value': {'key': 'semantic.value', 'type': 'semantic.value', 'value': 5, 'source_node': 'example'}}, 'params': {}},))
