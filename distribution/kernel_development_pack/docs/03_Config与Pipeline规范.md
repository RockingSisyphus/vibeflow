...
每个 root 默认读取自己的 `vibeflow_project.jsonc`，声明该 root 的 registry、quality 和可选的异步 runtime 参数。可用的 node/base_lib/plugin 都在该 root 的 `registry.py` 中声明：
...
`runtime` 只允许 `async_max_workers` 与 `async_flush_timeout`。前者必须是正整数，控制每个 Runtime 自有线程池的并发数并默认取 4；后者可以是 `null` 或非负秒数，用于 detached task 收尾。嵌套 Runtime 继承配置但不共享线程池。
