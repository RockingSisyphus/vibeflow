...
## CLI 让渡模式 / delegate-cli 的 Pipeline 契约

`delegate-cli` 把同一套 workflow 作为普通业务 CLI 运行。它不需要新 config schema，但 pipeline 必须显式声明固定入口和退出契约：
...
```jsonc
{
  "pipeline": {
    ...
    "nodes": [
      {
        "id": "parse_cli",
        "type_used": "app.parse_cli",
        "display_name": "Parse CLI",
        "description": "Parses delegated business arguments from cli.argv.",
        "requires": [{"type": "cli.argv", "cardinality": "exactly_one", "display_name": "CLI Arguments"}],
        "provides": [
          {"key": "cli.input_path", "type": "cli.input_path", "display_name": "CLI Input Path"},
          {"key": "cli.verbose", "type": "cli.verbose", "display_name": "CLI Verbose"}
        ]
      },
      {
        "id": "exit_cli",
        "type_used": "app.exit_cli",
        "display_name": "Exit CLI",
        "description": "Returns the business process exit code.",
        "requires": [{"type": "result.status", "cardinality": "exactly_one", "display_name": "Result Status"}],
...
class ParseCliNode:
    NODE_INFO = NodeInfo(
        type_key="app.parse_cli",
        display_name="Parse CLI",
        category="cli",
        description="Parses delegated business arguments.",
        version="1.0.0",
        flow_kind="io",
    )
    ...
    def run_pure(self, inputs, params):
        ...
        argv = list(inputs["cli.argv"]["value"])
        parsed = parser.parse_args(argv)
        return {
            "cli.input_path": parsed.input,
            "cli.verbose": parsed.verbose,
        }
```

- `cli.argv` 的业务值是首个 `--` 分界后全部 token，加上分界前未被 core 识别的 token，类型为 `list[str]`。分界可省略，且内核不得改写或重排让渡参数。
- `pipeline.outputs` 必须对 `cli.exit_code` 声明 `cardinality="exactly_one"`。最终必须恰有一个 provider，其 key 和 type 都是 `cli.exit_code`。
- `cli.exit_code` 的值必须是非 bool 的 `int`，范围为 `0..255`。不要输出 stdout/stderr 包装对象来替代真实标准流。
- 解析业务参数的 node 通常是 `flow_kind=io`，因而获得 `effect_scope=terminal` 使用 `argparse` 和标准流。普通 `process` 不能偷读 `sys.argv`。
- 业务 `SystemExit` 只能由 `io` / `document` / `data_store` node 或 runtime plugin 发出；其他位置的 `SystemExit` 是未授权框架错误。

详细标准流、日志与退出码语义见 `07_启动命令与报告.md`。`delegate-cli` 不改变通用 `run` 和架构审核 `review` 的契约。
