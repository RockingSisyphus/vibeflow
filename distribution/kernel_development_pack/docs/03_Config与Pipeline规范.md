...
每个 root 默认读取自己的 `vibeflow_project.jsonc`，声明该 root 的 registry、base_lib、plugins 和 quality 开关：
...
workspace 模式下，registry 按 `roots` 顺序加载，但 Python node 的 `type_key`、nodeset 的 `type_key` 共享同一个全局命名空间，不能重复。`base_lib.paths`、plugin 文件路径和 registry 文件路径都相对其所属 root 目录解析。pipeline config 不再声明 `policy`、`base_lib` 或 `plugins`；这些字段应迁移到根目录 `vibeflow_config.jsonc` 或对应 root 的 `vibeflow_project.jsonc`。
...
无 workspace 配置时，旧的单 project 模式仍兼容在 pipeline config 顶层声明 `base_lib`、`plugins` 和 `policy`。新项目不建议继续使用这种写法。

## 顶层资源声明

`global_config` 是 pipeline config 的运行参数元数据。无 workspace 的旧单 project 模式下，`base_lib`、`plugins` 也可以作为 config 资源元数据出现；workspace 模式下它们必须放在所属 root 的 `vibeflow_project.jsonc`。这些资源不会进入 `GraphConfig.nodes`、execution plan、`effective_edges` 或孤儿节点/可达性等 flow health 检查。Mermaid 会把它们画成独立资源区。
...
- workspace 模式下 `base_lib.paths` 是相对所属 root 目录的搜索路径；无 workspace 的旧模式下才相对当前 pipeline config 文件解析。
