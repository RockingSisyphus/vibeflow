...
`display_name`、`category`、`version`、`description`、`style` 是可视化元数据，不会进入运行时 `params`。如果 node 运行时确实需要同名参数，必须写进 `"config": {...}`。

Mermaid/SVG label 默认以可读性优先，使用纯文本分区展示：`name:`、`type:`，空行后展示 `display:`/`desc:` 等元数据，再空行展示 `requires:`/`provides:`。长说明会确定性换行并在必要时截断。SVG 渲染使用更大的 node spacing、rank spacing、wrapping width 和 diagram padding；普通图和展开审查图都按可读优先生成。
